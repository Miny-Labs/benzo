...
const TEST_AUTH_AUD = "benzo:console";
...
export function createTestAuthToken(input: { subject?: string; email?: string; name?: string; ttlSeconds?: number } = {}): string {
  ...
  const payload: GoogleClaims = {
    iss: "benzo:test",
    aud: TEST_AUTH_AUD,
    sub: input.subject || "codex-vps-console",
    email: input.email,
    email_verified: true,
    name: input.name || "Codex VPS Console",
    exp: now + ttl,
  };
...
export async function authFromRequest(req: IncomingMessage): Promise<AuthContext | null> {
  ...
  if (testClaims) {
    const key = createHash("sha256").update(`console|${testClaims.iss}|${testClaims.aud}|${testClaims.sub}`).digest("hex").slice(0, 32);
